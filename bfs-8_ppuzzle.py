...
def bfs(initial_state):
    visited = set()
    queue = deque([(initial_state, "", find_blank(initial_state))])  
    while queue:
        state, path, blank_pos = queue.popleft()
        ...
        for move, direction in zip(moves, "UDLR"):
            ...
            if new_state and tuple(map(tuple, new_state)) not in visited:
                queue.append((new_state, path + direction, new_blank))
    return None
...
if solution:
    print("Solution found using BFS:", solution)
else:
